...
同时必须读取 [references/firmware-ux.md](https://github.com/rfdiosuao/summon-protocol/blob/main/skills/summon-device-onboarding/references/firmware-ux.md)：支持 SoftAP 的 Wi-Fi 固件必须提供手机连接设备热点后可访问的本地页面，配置目标 Wi-Fi 与 Agent 铭牌；每个非根页面必须可返回，有确认键时双击确认必须返回且不能先触发单击动作。不支持热点的硬件必须明确缺口与替代路径，不能报告原生配网通过。铭牌后端未实现时标记待接入，不能编造接口。
...
开始设计时必须阅读 [配网、铭牌与返回交互要求](https://github.com/rfdiosuao/summon-protocol/blob/main/skills/summon-device-onboarding/references/firmware-ux.md)，把手机本地配网、铭牌字段、双击确认返回和对应验收加入固件交付范围。不能等刷写完成后才补设计。
...
- 铭牌后端与电脑客户端已实现，读取 https://github.com/rfdiosuao/summon-protocol/blob/main/docs/NAMEPLATES.md 和对应 nameplate.schema.json；通过精确查询、设备配对授权和正式会话接口接入。固件本身仍需实现本地页面与接口适配。开发前检查所部署 Hub 是否支持这些端点；旧服务器缺少接口时显示“待接入”，不编造 /resolve 等接口，也不把测试替身当生产功能。
