...
外部接入者可到 https://github.com/rfdiosuao/summon-protocol/issues/new 提交“设备接入申请”，附型号、SDK 链接、期望能力和脱敏测试结论，请维护者指定私密发放渠道。不要公开任何 token、访问码或私人设备标识。评估与开发可先完成，正式联网需获发凭证，不能承诺只粘贴提示词就自动注册任何硬件。

当前网络由部署者配置设备身份与 Gateway token；安装包不含凭证。Agent 自助注册无需邀请码，但这不等于获得设备 token 或 Gateway 授权。可用本地模拟先开发，不能据此宣称设备已上线。
