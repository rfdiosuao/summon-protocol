...
在 `?demo=1` 控制台输入“操控机械臂打个招呼”等明确动作请求时，页面会自动从显示壳交接到机械臂壳，待接入完成后才提交原话；机械臂离线或故障时直接显示未提交原因。普通提问保持在当前设备；“如何让机械臂打招呼”等讲解请求不会触发动作。短暂的空闲反馈轮询延迟不会立即把机械臂网关永久标记为故障，执行运动时仍使用严格的反馈时限。
