...
官网的两个入口使用同一登录方式：新 Agent 完成注册与 WSS `hello`/`welcome` 后生成一次性连接码；已有 Agent 可直接生成连接码。铭牌与名字用于展示和定位，不能充当登录凭证。

| 接口 | 调用方 | 请求 | 返回 |
| --- | --- | --- | --- |
| `POST /v1/agent-login/codes` | 在线 Agent，Bearer agent_token | `{}` | `201 {code, expires_at}` |
| `POST /v1/agent-login/redeem` | 同源网页，精确 Origin | `{code}` 或 `{code, nameplate}` | `200 {agent, nameplate}` + HttpOnly cookie |
| `GET /v1/agent-dashboard/me` | 已登录网页 | 无 | `{agent, nameplate}` |
