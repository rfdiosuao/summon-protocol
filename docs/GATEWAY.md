...
适配器提供 `arm.observe`（六轴实测角、末端位置、负载和模型净空）、`arm.motion`（模型提出短路点）与 `arm.gesture`（本地已录制预设）。远端 Agent 不能传串口帧；自由动作只可使用笔记本配置 `motion_bounds` 中已验证的 J1–J6 窗口和 `max_motion_speed_dps` 上限。每段最多 2–4 个相对路点，末点回到起始角；单轴偏移上限由 `max_relative_offset_deg` 设定，默认 10°、最大 30°。动作可用统一速度 0.5–10°/s，或为各运动轴分别设速 0.2–10°/s；`motion_axis_speed_caps_dps` 可进一步限制各轴，预估整段最多 6.5 秒。执行前读取真实关节角，以仓库 DM URDF/STL 按每不超过 1° 检查直接关节插值，分速动作还按各轴到达时间检查模型路径；模型重合、触桌或低于本地已验证 `min_clearance_mm` 的轨迹均拒绝，默认阈值为 20 mm。随后重新读取实机姿态确认未漂移，按路点下发，运动中持续核对实测模型边界和未控制轴漂移，并等待真实角度、速度和目标值回执。失败、断线或租约撤销时调用控制台的停止并保持命令，且必须确认姿态稳定才向 Hub 回停止成功。机械臂断开、仿真、反馈过期或故障时拒绝上线。[实机演示、主臂遥操与录制命令](ARM-LIVE-DEMO.md) 给出了本地 Hub 和六轴控制流程。
