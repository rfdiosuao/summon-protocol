...
框架只能上报 `ESTOP`，不能代替它。运动设备须具备经过验证的本地停止与保护路径；屏幕、手机等无运动设备不强制配机械急停，按 [硬件准入标准](HARDWARE-ADMISSION.md) 验收。
...
> 阻塞式渲染/播放**必须**另起工作任务。回调接口表达异步完成意图，但无法阻止实现内部阻塞，仍需代码审查与实测。
...
| 2 | PCM 阻塞必须进工作任务 | `audio_pcm` 回调表达完成；具体任务调度须由移植者实现并验证 |
...
异步任务、音频单持有者、真实完成回调与内存观测均需移植者验证；接口本身无法证明驱动正确。

## 四、框架替你持有的安全规则

这些规则在 `components/summon_device/` 中实现；源码可修改，因此修改后仍需运行对应验证，不能视为不可绕过的保护：
